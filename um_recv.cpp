...
int main(int argc, char* argv[]) {
    ...
    uint32_t um_matrix_num = 12, um_rows = 2, um_cols = 12;
    ...
    while (test_times--) {
        ...
        // 算子处理
        um_matrixs =
            array_to_matrixs(rdma_array, array_len, um_matrix_num, um_rows);

        uint32_t src_ip = (192 << 24) | (168 << 16) | (8 << 8) | 1;
        uint32_t dst_ip = (192 << 24) | (168 << 16) | (8 << 8) | 2;
#if OP_MAX
        uint32_t max_data = operator_max(um_matrixs, src_ip, dst_ip);
#endif  // OP_MAX
#if OP_MIN
        uint32_t min_data = operator_min(um_matrixs, src_ip, dst_ip);
#endif  // OP_MIN
#if OP_EXIST
        bool exist = operator_exist(um_matrixs, src_ip, dst_ip);
#endif  // OP_EXIST
#if OP_DISTINCT
        uint32_t distinct = operator_distinct(um_matrixs);
#endif  // OP_DISTINCT
#if OP_MERGE
        Matrixs merged = operator_merge({um_matrixs, um_matrixs});
#endif  // OP_MERGE
#if OP_FILTER
        auto filter = [](uint32_t i) { return i > 1000; };
        std::vector<
            std::unordered_map<std::pair<int, int>, uint32_t, pair_hash>>
            filtered = operator_filter(um_matrixs, filter);
#endif  // OP_FILTER
