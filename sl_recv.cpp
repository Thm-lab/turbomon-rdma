...
int main(int argc, char* argv[]) {
    ...
    uint32_t sl_matrix_num = 8, sl_rows = 8, sl_cols = 78125;
    ...
    while (test_times--) {
        ...
        // 算子处理
        sl_matrixs =
            array_to_matrixs(rdma_array, array_len, sl_matrix_num, sl_rows);

        uint32_t src_ip = (192 << 24) | (168 << 16) | (8 << 8) | 1;
        uint32_t dst_ip = (192 << 24) | (168 << 16) | (8 << 8) | 2;
#if OP_MAX
        uint32_t max_data = operator_max(sl_matrixs, src_ip, dst_ip);
#endif  // OP_MAX
#if OP_MIN
        uint32_t min_data = operator_min(sl_matrixs, src_ip, dst_ip);
#endif  // OP_MIN
#if OP_EXIST
        bool exist = operator_exist(sl_matrixs, src_ip, dst_ip);
#endif  // OP_EXIST
#if OP_DISTINCT
        uint32_t distinct = operator_distinct(sl_matrixs);
#endif  // OP_DISTINCT
#if OP_MERGE
        Matrixs merged = operator_merge({sl_matrixs, sl_matrixs});
#endif  // OP_MERGE
#if OP_FILTER
        auto filter = [](uint32_t i) { return i > 1000; };
        std::vector<
            std::unordered_map<std::pair<int, int>, uint32_t, pair_hash>>
            filtered = operator_filter(sl_matrixs, filter);
#endif  // OP_FILTER
