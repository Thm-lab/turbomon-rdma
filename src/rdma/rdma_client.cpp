...
rdma_client::rdma_client(const char* ip,
                         int port,
                         void* start_buf,
                         int start_size,
                         void* rdma_buf,
...
void rdma_client::bindaddr() {
    ...
    ret =
        rdma_resolve_addr(this->cm_id, NULL, (struct sockaddr*)this->sin, 2000);
    if (ret) {
        std::cerr << "rdma_reslove_addr error: " << strerror(errno)
                  << std::endl;
        exit(1);
    }
...
void rdma_client::cm_thread() {
    ...
    while (true) {
        ...
        if (ret) {
            std::cerr << "rdma_get_cm_event error: " << strerror(errno)
                      << std::endl;
            exit(1);
        }
        switch (event->event) {
            case RDMA_CM_EVENT_ADDR_RESOLVED:
                ...
                if (ret) {
                    std::cerr << "rdma_resolve_route error: " << strerror(errno)
                              << std::endl;
                    exit(1);
                }
                state = ADDR_RESOLVED;
        ...
        if (ret) {
            std::cerr << "rdma_ack_cm_event error: " << strerror(errno)
                      << std::endl;
            exit(1);
        }
    }
    ...
    if (wc.wc_flags & IBV_WC_WITH_IMM) {
        ...
        switch (imm_data) {
            case 1122:
                ...
                std::cout << "接受到远程地址信息 addr: " << remote_addr
                          << std::endl;
                std::cout << "接受到远程地址信息 len: " << remote_len
                          << std::endl;
                std::cout << "接受到远程地址信息 rkey: " << remote_rkey
                          << std::endl;
                GET_RDMA_ADDR = true;
                break;
            default:
...
void rdma_client::start() {
    ...
    // 创建监听cm
    std::thread cmthread([this]() { this->cm_thread(); });
    cmthread.detach();
    ...
    if (!channel) {
        std::cerr << "ibv_create_comp_channel error: " << strerror(errno)
                  << std::endl;
        exit(1);
    }
    ...
    ret = ibv_req_notify_cq(cq, 0);
    if (ret) {
        std::cerr << "ibv_req_notify_cq error: " << strerror(errno)
                  << std::endl;
        exit(1);
    }
    ...
    // 启动cq
    std::thread cqthread([this]() { this->cq_thread(); });
