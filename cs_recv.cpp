...
int main(int argc, char* argv[]) {
    ...
    uint32_t cs_rows = 1, cs_cols = 312500;
    ...
    while (test_times--) {
        ...
        // 算子处理
        cs_matrix = array_to_matrix(rdma_array, array_len, cs_rows);

        uint32_t src_ip = (192 << 24) | (168 << 16) | (8 << 8) | 1;
        uint32_t dst_ip = (192 << 24) | (168 << 16) | (8 << 8) | 2;
#if OP_MAX
        uint32_t max_data = operator_max(cs_matrix, src_ip, dst_ip);
#endif  // OP_MAX
#if OP_MIN
        uint32_t min_data = operator_min(cs_matrix, src_ip, dst_ip);
#endif  // OP_MIN
#if OP_EXIST
        bool exist = operator_exist(cs_matrix, src_ip, dst_ip);
#endif  // OP_EXIST
#if OP_DISTINCT
        uint32_t distinct = operator_distinct(cs_matrix);
#endif  // OP_DISTINCT
#if OP_MERGE
        Matrix merged = operator_merge({cs_matrix, cs_matrix});
#endif  // OP_MERGE
#if OP_FILTER
        auto filter = [](uint32_t i) { return i > 1000; };
        std::unordered_map<std::pair<int, int>, uint32_t, pair_hash> filtered =
            operator_filter(cs_matrix, filter);
#endif  // OP_FILTER
